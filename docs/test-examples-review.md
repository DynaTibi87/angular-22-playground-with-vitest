...
- **Suggestion (minor):** The identity chain assertions
  ...
  example's _topic_ is injection.
...
- **Weak assertion (fix recommended):** In "should never touch the expensive
  service", the spy is created _after_ the component is already rendered:
  ```ts
  ...
  that _had_ happened. The assertion therefore can't fail. To make it meaningful,
